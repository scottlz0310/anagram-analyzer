...
- [ ] 候補詳細画面（漢字表記・意味表示）
- [x] 候補詳細ダイアログの最小実装（読み表示 + 漢字/意味プレースホルダ）
- [x] 候補詳細ダイアログで漢字/意味のseed実データ表示（未収録語はプレースホルダ）
...
| 4: UI実装 | 🟡 進行中 | メイン画面実装、候補詳細ダイアログ最小実装+seed実データ表示、ライト/ダーク切替、ランチャーアイコン適用、手動テスト可能な最小フロー、Compose UIテスト追加 |
| 5: 辞書データ | 🟡 進行中 | seed変換/取込導線 + サイズ最適化（`max-len=8`）+ ライセンス表示 + 初回インポート計測ログ + 8/10投入時間比較まで実施 |
| 6: 追加機能 | 🟡 進行中 | DataStore によるテーマ設定永続化 + 入力履歴永続化 + 履歴折りたたみ表示 + 設定画面（文字数範囲/テーマ/追加辞書DL項目）まで実装 |
