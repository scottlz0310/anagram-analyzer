...
- [ ] Androidプロジェクト雛形作成（Gradle Kotlin DSL）
- [ ] パッケージ構成（data / domain / ui）のスケルトン作成
- [ ] Room DB セットアップ（AnagramEntry Entity + DAO）
- [ ] Hilt DI 基盤設定
- [ ] GitHub Actions にAndroidビルドジョブ追加

## フェーズ 3: コアロジック移植（Kotlin）

- [ ] ひらがな正規化（NFKC / カタカナ→ひらがな / バリデーション）
- [ ] アナグラムキー生成
- [ ] Room DAOによるインデックス検索
- [ ] ユニットテスト（JUnit）で Python版との動作一致を検証

## フェーズ 4: UI実装（Jetpack Compose）

- [ ] メイン画面（テキスト入力 + 検索 + 候補リスト）
- [ ] 候補詳細画面（漢字表記・意味表示）
- [ ] テーマ設定（Material 3）
